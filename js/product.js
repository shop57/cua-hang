...
function renderProduct(p) {
  ...
  if (stockBadge) {
    stockBadge.textContent = outOfStock ? "Hết hàng" : "Còn hàng";
    stockBadge.className = outOfStock ? "pd-stock-badge pd-stock-out" : "pd-stock-badge pd-stock-in";
  }
  if (addBtn) {
    addBtn.disabled = outOfStock;
    addBtn.textContent = outOfStock ? "Hết hàng" : "Thêm vào giỏ";
  }
  if (buyBtn) {
    buyBtn.disabled = outOfStock;
    buyBtn.textContent = outOfStock ? "Hết hàng" : "Mua ngay";
  }
...
function renderOptions(p) {
  ...
  const rawValues = Array.isArray(p.optionValues) ? p.optionValues : [];
  // Support legacy plain-string entries alongside the newer
  // { value, outOfStock } shape.
  const values = rawValues.map((v) => (typeof v === "string" ? { value: v, outOfStock: false } : v));
  ...
  listEl.innerHTML = values
    .map(
      (v, i) =>
        `<button type="button" class="pd-option-btn${v.outOfStock ? " pd-option-oos" : ""}" data-option-index="${i}" ${v.outOfStock ? "disabled" : ""}>${escapeHtml(v.value)}${v.outOfStock ? " <span class=\"pd-option-oos-label\">(Hết hàng)</span>" : ""}</button>`
    )
    .join("");

  listEl.querySelectorAll(".pd-option-btn:not(.pd-option-oos)").forEach((btn) => {
    btn.addEventListener("click", () => {
      listEl.querySelectorAll(".pd-option-btn").forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      selectedOption = values[Number(btn.dataset.optionIndex)].value;
    });
  });
}
...
function confirmOptionSelected() {
  if (currentProduct && currentProduct.stock === "out_of_stock") return false;
  const rawValues = currentProduct && Array.isArray(currentProduct.optionValues) ? currentProduct.optionValues : [];
  if (rawValues.length && !selectedOption) {
    const wrap = document.getElementById("pdOptions");
