...
function renderOptionGroup({ groupId, listId, values, selectedGetter, selectedSetter }) {
  ...
  group.hidden = false;
  selectedSetter(items[0]);
  list.innerHTML = items
    .map(
      (v, i) =>
        `<button type="button" class="pd-option-btn${i === 0 ? " active" : ""}" data-value="${escapeHtmlProduct(v)}">${escapeHtmlProduct(v)}</button>`
    )
    .join("");

  list.querySelectorAll(".pd-option-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      list.querySelectorAll(".pd-option-btn").forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      selectedSetter(btn.getAttribute("data-value"));
    });
  });
}
...
function renderProduct(p) {
  ...
  renderOptionGroup({
    groupId: "pdVariantGroup",
    listId: "pdVariantList",
    values: p.variants,
    selectedSetter: (v) => (selectedVariant = v)
  });
  ...
  renderOptionGroup({
    ...
    listId: "pdSizeList",
    values: p.sizes,
    selectedSetter: (v) => (selectedSize = v)
  });
...
function attachInteractionHandlers() {
  ...
  document.getElementById("addToCartBtn").addEventListener("click", (e) => {
    Cart.add(currentProduct.id, currentQty);
  ...
  document.getElementById("buyNowBtn").addEventListener("click", () => {
    ...
    try {
      sessionStorage.setItem(
        ...
        JSON.stringify({ id: currentProduct.id, qty: currentQty })
      );
    } catch (e) {}
