...
const ProductStore = {
  ...
  // Cleans the option values array from the admin form: each entry is
  // { value, outOfStock }. Also accepts legacy plain-string entries
  // (saved before per-value stock tracking existed) and upgrades them.
  _sanitizeOptionValues(list) {
    if (!Array.isArray(list)) return [];
    return list
      .map((v) => {
        if (typeof v === "string") return { value: v.trim(), outOfStock: false };
        if (v && typeof v === "object") return { value: String(v.value || "").trim(), outOfStock: !!v.outOfStock };
        return null;
      })
      .filter((v) => v && v.value);
  },
  ...
  add(productData) {
    ...
    const product = {
      ...
      stock: productData.stock === "out_of_stock" ? "out_of_stock" : "in_stock",
      ...
      optionLabel: (productData.optionLabel || "").trim(),
      optionValues: this._sanitizeOptionValues(productData.optionValues),
  ...
  update(id, patch) {
    ...
    if (patch.stock !== undefined) product.stock = patch.stock === "out_of_stock" ? "out_of_stock" : "in_stock";
    ...
    if (patch.optionLabel !== undefined) product.optionLabel = String(patch.optionLabel).trim();
    if (patch.optionValues !== undefined) product.optionValues = this._sanitizeOptionValues(patch.optionValues);
  ...
  importJSON(jsonString) {
    ...
    parsed.forEach((item) => {
      ...
      const product = {
        ...
        stock: item.stock === "out_of_stock" ? "out_of_stock" : "in_stock",
        ...
        optionLabel: item.optionLabel ? String(item.optionLabel).trim() : "",
        optionValues: this._sanitizeOptionValues(item.optionValues),
