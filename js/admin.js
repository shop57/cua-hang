...
// ---------------- Option values (size/color) dynamic rows ----------------

function renderOptionValueRows(values) {
  const wrap = document.getElementById("optionValuesList");
  const rows = values && values.length ? values : [];
  wrap.innerHTML = rows
    .map(
      (v, i) => `
      <div class="option-value-row" data-row-index="${i}">
        <input type="text" class="opt-value-input" value="${escapeHtml(v.value || "")}" placeholder="VD: M">
        <label class="opt-value-outofstock">
          <input type="checkbox" class="opt-value-oos" ${v.outOfStock ? "checked" : ""}> Hết hàng
        </label>
        <button type="button" class="opt-value-remove" aria-label="Xoá lựa chọn">✕</button>
      </div>`
    )
    .join("");

  wrap.querySelectorAll(".opt-value-remove").forEach((btn) => {
...
function addOptionValueRow() {
  ...
  div.innerHTML = `
    <input type="text" class="opt-value-input" placeholder="VD: M">
    <label class="opt-value-outofstock">
      <input type="checkbox" class="opt-value-oos"> Hết hàng
    </label>
    <button type="button" class="opt-value-remove" aria-label="Xoá lựa chọn">✕</button>`;
  div.querySelector(".opt-value-remove").addEventListener("click", () => div.remove());
  wrap.appendChild(div);
  div.querySelector(".opt-value-input").focus();
}

function readOptionValueRows() {
  return Array.from(document.querySelectorAll("#optionValuesList .option-value-row"))
    .map((row) => ({
      value: row.querySelector(".opt-value-input").value.trim(),
      outOfStock: row.querySelector(".opt-value-oos").checked
    }))
    .filter((v) => v.value);
}
...
function readForm() {
  return {
    ...
    optionValues: readOptionValueRows(),
...
function fillForm(p) {
  ...
  document.getElementById("fStock").value = p.stock === "out_of_stock" ? "out_of_stock" : "in_stock";
  ...
  // Older products may have saved optionValues as plain strings — normalize
  // to {value, outOfStock} objects for the row editor.
  const rawValues = p.optionValues || [];
  const normalized = rawValues.map((v) =>
    typeof v === "string" ? { value: v, outOfStock: false } : { value: v.value || "", outOfStock: !!v.outOfStock }
  );
  renderOptionValueRows(normalized);
    ...
    .map((p) => {
      ...
      <div class="admin-row" data-row-id="${p.id}">
        ...
        <div class="admin-row-info">
          <p class="admin-row-name">${escapeHtml(p.name)} <span class="admin-row-id">#${p.id}</span> ${p.stock === "out_of_stock" ? `<span class="admin-row-outofstock">Hết hàng</span>` : ""}</p>
...
function initAdmin() {
  ...
  document.getElementById("addOptionValueBtn").addEventListener("click", () => {
    addOptionValueRow();
  });
  renderOptionValueRows([]);
