...
function readForm() {
  return {
    ...
    sizes: parseCommaList(document.getElementById("fSizes").value),
    variants: parseCommaList(document.getElementById("fVariants").value),
...
function renderOrdersList() {
  ...
  wrap.innerHTML = orders.map((o) => {
    const itemsSummary = o.items.map((i) => `${escapeHtml(i.name)} ×${i.qty}`).join(", ");
