...
const Cart = {
  ...
  add(productId, qty = 1, option = "") {
    const existing = this.items.find((i) => i.id === productId);
    if (existing) {
      existing.qty += qty;
      if (option) existing.option = option;
    } else {
      this.items.push({ id: productId, qty, option: option || "" });
    }
    this.save();
    this.renderAll();
  },
        ...
        return `
          <div class="cart-line" data-line-id="${p.id}">
            ...
            <div class="cart-line-info">
              ...
              ${item.option ? `<p class="cart-line-option">${escapeHtml(item.option)}</p>` : ""}
